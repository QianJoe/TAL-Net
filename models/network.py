...
class TLNet(nn.Module):
    ...
    def _init_module(self):
        ...
        self.metric_layer = TLModule(**self.kwargs)
...
if __name__ == '__main__':
    model = TLNet(base_model='Conv64F', inplanes=64)
    ...
    # model = TLNet(base_model='ResNet', inplanes=64)
    # query_data = torch.rand(75, 3, 84, 84)
    # support_data = [torch.rand(25, 3, 84, 84)]
    # result = model(query_data, support_data)
    # trainable_num = sum(p.numel() for p in model.parameters() if p.requires_grad)
